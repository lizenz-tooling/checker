...
// biome-ignore lint/suspicious/noExplicitAny: Preserve nopt's deliberately permissive parsed argument values.
type LegacyValue = any;
type ParsedArguments = Record<string, LegacyValue>;
...
function normalizeArgv(args: LegacyValue): LegacyValue {
 ...
 * @returns number
 */
function toDepthNumber(value: boolean | number | string | undefined = Number.POSITIVE_INFINITY): number {
	switch (typeof value) {
		case 'string':
...
function depthStringToNumber(depthString: string): number {
	let numberValue: number | string = depthString.toLowerCase();
	...
	if (numberValue === 'true') {
		...
		numberValue = (Number as LegacyNumberConstructor)(numberValue, 10);
 ...
 */
export function setDefaultArguments(parsedArguments: ParsedArguments = {}): ParsedArguments {
...
export function getNormalizedArguments(args?: string[]): ParsedArguments {
