...
type LegacyErrorConstructor = new (message: string, fileName: string, lineNumber: number) => Error;

export const detectLicenseTitle = (str = 'undefined'): string | null => {
	if (typeof str !== 'string') {
		throw new (Error as unknown as LegacyErrorConstructor)(
	...
	if (APACHE_VERSION.test(str)) {
		match = APACHE_VERSION.exec(str) as RegExpExecArray;
	...
	if (APACHE_VERSION_SHORT.test(str)) {
		match = APACHE_VERSION_SHORT.exec(str) as RegExpExecArray;
		version = match[1] as string;
	...
	if (GPL.test(str)) {
		match = GPL.exec(str) as RegExpExecArray;
		version = match[1] as string;
	...
	if (LGPL.test(str)) {
		match = LGPL.exec(str) as RegExpExecArray;
		version = match[1] as string;
	...
	if (match) {
		const matchedUrl = match[1] as string;
