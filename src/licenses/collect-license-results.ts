...
const INITIAL_MODULE_INFO: LegacyRecord = { licenses: LICENSE_TITLE_UNKNOWN };

const getPackageNameAndVersion = (packageJson: LegacyRecord): string => `${packageJson.name}@${packageJson.version}`;

const findClarification = (clarifications: LegacyRecord, packageJson: LegacyRecord): LegacyValue =>
	clarifications[packageJson.name]?.find(
		(clarification: LegacyRecord) =>
			packageJson.version === clarification.semverRange ||
			semver.satisfies(packageJson.version, clarification.semverRange)
	);

const shouldSkipPackage = ({ packageJson, data, options }: LegacyRecord): LegacyValue =>
...
const collectPackageLicenseInfo = ({
	packageJson: currentExtendedPackageJson,
	data,
	options,
}: LegacyRecord): boolean => {
	const moduleInfo: LegacyRecord = { ...INITIAL_MODULE_INFO };
	...
	const mustInclude = (propertyName = ''): boolean => options?.customFormat?.[propertyName] !== false;
	...
	if (licenseData) {
		...
		if (Array.isArray(licenseData) && licenseData.length > 0) {
			...
			moduleInfo.licenses = licenseData.map((moduleLicense: LegacyValue) => {
	...
	licenseFilesInCurrentModuleDirectory.forEach(function findBetterLicenseData(filename, index) {
		...
		if (fs.lstatSync(licenseFile).isFile()) {
			let currentLicenceFilesContent: string | undefined;
